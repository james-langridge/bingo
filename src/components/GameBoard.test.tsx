...
describe('GameBoard', () => {
  ...
  describe('marking items', () => {
    ...
    test('updates styling when marked positions change', () => {
      const { rerender } = render(<GameBoard {...defaultProps} markedPositions={[0]} />);
  ...
  describe('text display', () => {
    test('handles long text items', () => {
      const longTextItems = [
        { id: '1', text: 'This is a very long text that should wrap properly in the button', position: 0 },
      ];

      render(<GameBoard {...defaultProps} items={longTextItems} gridSize={1} />);
