...
describe('storage', () => {
  ...
  describe('loadGameByCode', () => {
    ...
    test('returns correct game when multiple games exist', async () => {
      const game2 = { ...mockGame, id: 'game-456', gameCode: 'DEF456', title: 'Game 2' };
      await saveGameLocal(mockGame);
      await saveGameLocal(game2);
  ...
  describe('loadGameByAdminToken', () => {
    ...
    test('returns game with matching admin token', async () => {
      await saveGameLocal(mockGame);
      const game = await loadGameByAdminToken('admin123456789012345678901234567');
      expect(game).toEqual(mockGame);
    });
    ...
    test('returns correct game when multiple games exist', async () => {
      ...
      const found = await loadGameByAdminToken('different12345678901234567890123');
      expect(found?.title).toBe('Game 2');
    });
  });
  ...
  describe('queueEvent', () => {
    ...
    test('handles different event types', async () => {
      const events: GameEvent[] = [
        { type: 'ITEM_ADDED', itemId: '1', text: 'Text', timestamp: Date.now() },
        { type: 'ITEM_MARKED', itemId: '1', playerId: 'p1', timestamp: Date.now() },
        { type: 'ITEM_UNMARKED', itemId: '1', playerId: 'p1', timestamp: Date.now() },
